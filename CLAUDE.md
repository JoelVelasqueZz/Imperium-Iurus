...
| estado | text | `nuevo` · `leido` · `respondido` · `archivado` |
| created_at | timestamptz | |
...
| `/admin/contacto` | Gestión de consultas de contacto |
...
## Análisis de call graph con noodles (MCP)

El servidor MCP `noodles` (`pip install noodles`, tools `mcp__noodles__*`) genera un grafo de llamadas del repo vía AST (tree-sitter). Útil para mapear módulos, pero tiene una limitación concreta detectada en este repo:

**No resuelve el alias de TypeScript `@/*` → `./*`** (`tsconfig.json` línea `"paths": { "@/*": ["./*"] }`), que es como está escrito casi todo el import interno del proyecto. Efecto:

- **Falsos huérfanos:** funciones/componentes muy usados (`getSiteConfig`, `ConfigProvider`, `useSiteConfig`, `useUpdateConfig`, cualquier `page.tsx`/`layout.tsx`) aparecen sin llamadores en el grafo aunque el flujo real (ver sección "Sistema de configuración del sitio" arriba) sí exista y esté en uso en ~28 archivos.
- **Falsos positivos por colisión de nombre:** cuando dos funciones distintas comparten identificador, noodles a veces las conecta igual. Ejemplo verificado: `middleware.ts` y `app/chat/page.tsx` llaman a `supabase.auth.getUser()` (SDK de Supabase) y el grafo lo dibuja como llamada a `lib/supabase-server.ts::getUser` (función propia sin relación) — coincidencia de nombre, no de import real.

**Conclusión práctica:** usar `get_callers`/`get_callees`/`filter_graph` de noodles para tener un mapa aproximado rápido, pero para cadenas cross-file specíficas (sobre todo las que pasan por Context providers o alias `@/`) verificar siempre contra el código fuente antes de asumir el grafo como verdad.

### Exportar el viewer interactivo como archivo compartible

`analyze_local_repo` deja los resultados en una carpeta temporal (`result_path` de la respuesta) que incluye `viewer.html`. Ese archivo trae el grafo **embebido inline** (no depende del servidor para los datos) y solo carga una dependencia externa por CDN (`mermaid.min.js` vía jsdelivr, en el `<script src>` del `<head>`). Por eso:

- Se puede copiar y compartir como archivo suelto (WhatsApp, Drive, USB) — se abre con doble clic en cualquier navegador, sin Python ni servidor.
- Para un link público sin hosting propio: arrastrar el archivo a Netlify Drop (sin cuenta) o subirlo a GitHub Pages.
- **Publicado:** https://fascinating-dasik-c50921.netlify.app/ — snapshot del análisis `ce4824ca73e8` (2026-07-06). Al ser un snapshot estático, no se actualiza solo; si el código cambia mucho, regenerar con `analyze_local_repo` y volver a publicar.
- **No sirve como Artifact de Claude** — el CSP del sandbox de Artifacts bloquea scripts de CDN externos, así que Mermaid no cargaría y el diagrama quedaría en blanco.
- Si se quiere el visor con servidor local en vez del archivo suelto: `noodles viewer <result_dir> --port <N>` (el ejecutable puede no estar en el PATH si se instaló como paquete de usuario — buscar `noodles.exe` en la carpeta `Scripts` del `site-packages` de Python).
